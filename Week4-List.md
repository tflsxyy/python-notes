...
将一颗色子掷`6000`次，统计每个点数出现的次数，我们可以用`1`到`6`均匀分布的随机数来模拟掷色子，然后用`6`个变量分别记录每个点数出现的次数。
...
上面的代码，会显得非常的“笨重”和“丑陋”，更可怕的是，如果要统计掷两颗或者更多的色子统计每个点数出现的次数，那就需要定义更多的变量，写更多的分支结构。那么有没有办法用一个变量来保存多个数据，有没有办法用统一的代码对多个数据进行操作？答案是肯定的，在Python中我们可以通过容器类型的变量来保存和操作多个数据，我们首先为大家介绍列表（list）这种新的数据类型。
...
和字符串类型一样，列表也支持拼接、重复、成员运算、索引和切片。

```python
# 列表的正向索引和反响索引
item = ['red', 'green', 'blue', 'yellow', 'white', 'black']
print(item[0])
print(item[1])
print(item[2])
print(item[-1])
...
<img src="https://www.runoob.com/wp-content/uploads/2014/05/positive-indexes-1.png" width="80%">

<img src="https://www.runoob.com/wp-content/uploads/2014/05/negative-indexes.png" width="80%">
...
也可以统计完律师事务所的薪资后，使用列表来存储律所的名字和薪资水平，使用工具将薪资数据可视化。

```python
from matplotlib.font_manager import *
import matplotlib.pyplot as plt
myFont = FontProperties(fname="/System/Library/Fonts/STHeiti Medium.ttc") # 设置中文字体

law_firms = ['金杜律师事务所', '君合律师事务所', '中伦律师事务所', '海问律师事务所', \
            '方达律师事务所', '竞天公诚律师事务所', '通商律师事务所', '环球律师事务所']
salary = [1.9, 2.2, 2.2, 2.3, 2.5, 1.6, 1.3, 1.7]
variance = [0.1, 0.2, 0.2, 0.1, 0.1, 0.3, 0.2, 0.4]

plt.figure(figsize=(12.5, 5)) # 设置画布大小
plt.bar(law_firms, salary, yerr=variance) # 柱状图

plt.xlabel('律师事务所', fontproperties=myFont)
plt.ylabel('律所薪资 (万/月)', fontproperties=myFont)
plt.title('律所薪资待遇', fontproperties=myFont)
plt.xticks(law_firms, fontproperties=myFont)
plt.show()
```

### 列表的方法

和字符串一样，列表类型的方法也很多，下面是一些比较重要的方法。
...
从列表中删除元素其实还有一种方式，就是使用Python中的`del`关键字后面跟要删除的元素，这种做法跟使用`pop`方法指定索引删除元素没有实质性的区别。
...
下面这种方式不仅代码简单优雅，而且性能也优于上面使用`for`循环和`append`方法向空列表中追加元素的方式，记住“**强烈建议用生成式语法来创建列表**”这个结论就可以了。
